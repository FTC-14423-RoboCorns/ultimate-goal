...
public class ATestAuton extends OpMode {
    ...
    */
    ...
    public void init(){
        ...
        autonShooting=new AutonShooting(robot);
    ...
    public void start(){
        ...
        if (ringPosition==0) {
            ...
            trajectory1 = robot.drive.trajectoryBuilder(startPose)
                    ...
                    .build();
            autonShooting.setCurrentTarget(robot.shooter.redPowerShot1); //affects lift height - change if we shoot for goal
        } else if (ringPosition==1)
        {
            ...
            autonShooting.setCurrentTarget(robot.shooter.redGoal); //affects lift height - change if we shoot for goal
            ...
            trajectory1 = robot.drive.trajectoryBuilder(startPose)
                    ...
                    .build();
            //robot.shooter.currentTarget=robot.shooter.redPowerShot1; //affects lift height - change if we shoot for goal
            autonShooting.setCurrentTarget(robot.shooter.redGoal);
        ...
        switch (currentState) {
            ...
            case TRAJECTORY_1:
                if (!robot.shooter.isShooterOn) {
                    if (ringPosition==0) {
                        autonShooting.desiredVelocity=1950;
                    }
                        else {
...
********
            ...
            case DRIVE_WOBBLE_2:
                if (waitTimer1.time()>750) {
                    ...
                    autonWobble.setWobblePos(850);//630;
                    currentState = State.DROP_WOBBLE_2;
                }
                break;
            case DROP_WOBBLE_2:
